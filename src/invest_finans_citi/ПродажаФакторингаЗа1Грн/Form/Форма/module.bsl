...
Функция СоздатьДокОперация()
	...
	ДокОперация.Содержание = "Продажа Факторинга За 1 Грн";
	ДокОперация.Комментарий = "Продажа Факторинга За 1 Грн";
...
Функция СоздатьИЗаписатьНаборЗаписейПоСтроке(ТабличныйДокумент, ТекущаяСтрока, ДокОперация, РегХозрасчетныйСписаниеЗадолженности)	
	...
	КонецЦикла;
		...
		если СуммаОстаток <>  0 Тогда
			ВызватьИсключение("По Договору " +N_кредита+" остаток " + СуммаОстаток);
		КонецЕсли;	
	...
	Доход = 1;
		...
		РегЗапись.Содержание  = "Продажа Факторинга За 1 Грн";
