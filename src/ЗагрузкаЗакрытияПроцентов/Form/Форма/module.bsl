...
Функция СоздатьППВходящее(ТабличныйДокумент, ТекущаяСтрока)	
	...
	КонецПопытки;
		...
		Если ОперацияНачислениеПроцентов = Неопределено Или Месяц <> НачалоМесяца(Датакредита) Тогда
			...
			ДокОперация.Содержание = "начисление процентов ПОМЕСЯЧНО " + Формат(Датакредита, "ДФ=""ММ""" ) ;
		...
		РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("703");
...
Функция СоздатьДокОперация(ТипБухОперации)
