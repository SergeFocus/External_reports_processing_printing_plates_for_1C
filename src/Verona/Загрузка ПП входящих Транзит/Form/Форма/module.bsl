...
Функция ЗагрузитьДанные(ТабличныйДокумент, Индикатор) Экспорт
	...
	Начисление = Новый ТаблицаЗначений;
	//Начисление.Очистить();
	Начисление.Колонки.Добавить("ТипОперации");
	Начисление.Колонки.Добавить("ОперацияНачисление");
	Начисление.Колонки.Добавить("РегХозрасчетныйНачисление");
	Стр = Начисление.Добавить();
	Стр.ТипОперации = "Пеня";
	Стр = Начисление.Добавить();
	Стр.ТипОперации = "Проценты";
	
	ОперацияНачислениеПени = Неопределено;
	ОперацияНачислениеПроцентов = Неопределено;
	РегХозрасчетныйНачислениеПени = Неопределено;
...
Функция СоздатьППВходящее(ТабличныйДокумент, ТекущаяСтрока)	
	...
	Попытка	
		ЧтениеИПроверкаДанных(ТекущаяСтрока, N_кредита, ДатаОперации, ДоговорКонтрагента, Контрагент, Процент, Тело, ШтрафПеня, Сумма);
		...
		Если ОперацияДеньгиВпути = Неопределено Или ОперацияДеньгиВпути.Дата <>  НачалоДня(ДатаОперации)+4*60*60  Тогда
			...
			ДокОперация.Содержание = "Грошові кошти в дорозі в національній валюті ПЛАТОН";
			ДокОперация.Комментарий = "Грошові кошти в дорозі в національній валюті ПЛАТОН";
			ДокОперация.СпособЗаполнения = "Вручную";
			ДокОперация.Записать();
			ОперацияДеньгиВпути = ДокОперация.Ссылка;
			РегХозрасчетныйДеньгиВпути = РегистрыБухгалтерии.Хозрасчетный.СоздатьНаборЗаписей();
			РегХозрасчетныйДеньгиВпути.Отбор.Регистратор.Значение = ОперацияДеньгиВпути;
			Сообщить("Создана Операция Грошові кошти в дорозі в національній валюті ПЛАТОН за "+ ДокОперация.Дата);
		КонецЕсли; 
	//       
		...
		РегЗапись = РегХозрасчетныйДеньгиВпути.Добавить();
		РегЗапись.Период = ОперацияДеньгиВпути.Дата;
		РегЗапись.Регистратор = ОперацияДеньгиВпути;
		РегЗапись.Организация = глЗначениеПеременной("ОсновнаяОрганизация");
		РегЗапись.Содержание  = "Погашення кредиту";
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3331");
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", СчетОрганизации);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("1111 Погашення кредиту"));
		РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("37712");
		...
		РегЗапись.Сумма = Тело;
		РегХозрасчетныйДеньгиВпути.Записать();
		Опер = ОперацияДеньгиВпути.ПолучитьОбъект();
		...
		РегЗапись.Регистратор = ОперацияДеньгиВпути;
		РегЗапись.Организация = глЗначениеПеременной("ОсновнаяОрганизация");
		РегЗапись.Содержание  = "Нарахування % по кредиту";
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3331");
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", СчетОрганизации);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("1112 Відсотки за кредитом"));
		РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3731");
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Контрагенты", Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Договоры", ДоговорКонтрагента);
		РегЗапись.Сумма = Процент;
		РегХозрасчетныйДеньгиВпути.Записать();
		Опер = ОперацияДеньгиВпути.ПолучитьОбъект();
		Опер.СуммаОперации = Опер.СуммаОперации + РегЗапись.Сумма;
		Опер.Записать();

			ДобавитьВОперацию(ДатаОперации, ДоговорКонтрагента, Контрагент, "Проценты", "начисление % по кредиту", "Создана Операция Начисление Процентов за ", Процент, ПланыСчетов.Хозрасчетный.НайтиПоКоду("373"), ПланыСчетов.Хозрасчетный.НайтиПоКоду("703"));
		КонецЕсли;
		
		//Штраф за прострочення кредиту
		Если ШтрафПеня > 0 Тогда 
		РегЗапись = РегХозрасчетныйДеньгиВпути.Добавить();
		РегЗапись.Период = ОперацияДеньгиВпути.Дата;
		РегЗапись.Регистратор = ОперацияДеньгиВпути;
		РегЗапись.Организация = глЗначениеПеременной("ОсновнаяОрганизация");
		РегЗапись.Содержание  = "Штраф за прострочення кредиту";
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3331");
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", СчетОрганизации);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("1113 Штраф за прострочення кредиту"));
		РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3741");
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Контрагенты", Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Договоры", ДоговорКонтрагента);
		РегЗапись.Сумма = ШтрафПеня;
		РегХозрасчетныйДеньгиВпути.Записать();
		Опер = ОперацияДеньгиВпути.ПолучитьОбъект();
		Опер.СуммаОперации = Опер.СуммаОперации + РегЗапись.Сумма;
		Опер.Записать();
		
			ДобавитьВОперацию(ДатаОперации, ДоговорКонтрагента, Контрагент, "Пеня", "начисление Пени", "Создана Операция Начисление Пени за ", ШтрафПеня, ПланыСчетов.Хозрасчетный.НайтиПоКоду("374"), ПланыСчетов.Хозрасчетный.НайтиПоКоду("715"));
		КонецЕсли;
...
Процедура ДобавитьВОперацию(Знач ДатаОперации, ДоговорКонтрагента, Контрагент, Знач ТипОперации, Знач Содержание, Знач Сообщение, Знач Сумма, Знач СчетДт, Знач СчетКт)
	
	Перем Опер, РегЗапись;
	
	ТекущаяОперация = Начисление.Найти(ТипОперации);
	
	Если ТекущаяОперация.ОперацияНачисление = Неопределено Тогда
		СоздатьОперациюНачисления(ТекущаяОперация, ДатаОперации,  Содержание, Сообщение);
	КонецЕсли; 
	РегЗапись = ТекущаяОперация.РегХозрасчетныйНачисление.Добавить();
	РегЗапись.Период = КонецДня(ДатаОперации);
	РегЗапись.Регистратор = ТекущаяОперация.ОперацияНачисление;
	...
	ТекущаяОперация.РегХозрасчетныйНачисление.Записать();
	Опер = ТекущаяОперация.ОперацияНачисление.ПолучитьОбъект();
	Опер.СуммаОперации = Опер.СуммаОперации + РегЗапись.Сумма;
	Опер.Записать();
	
КонецПроцедуры

Процедура СоздатьОперациюНачисления(Знач ТекущаяОперация, Знач ДатаОперации, Знач Содержание, Знач Сообщение)
	...
	ДокОперация.СпособЗаполнения = "Вручную";
	ДокОперация.Записать();
	ТекущаяОперация.ОперацияНачисление = ДокОперация.Ссылка;
	ТекущаяОперация.РегХозрасчетныйНачисление = РегистрыБухгалтерии.Хозрасчетный.СоздатьНаборЗаписей();
	ТекущаяОперация.РегХозрасчетныйНачисление.Отбор.Регистратор.Значение = ТекущаяОперация.ОперацияНачисление;
	Сообщить(Сообщение+ ДокОперация.Дата);
	
КонецПроцедуры

Функция ЗаполнениеППВх(Знач ДатаОперации, Знач ДоговорКонтрагента, Знач Контрагент, Знач ПараметрОбъектКопирования, Знач Сумма, СтатьяДвиженияДенежныхСредств, СчетУчетаРасчетовСКонтрагентом, Комментарий)
	Перем НоваяРасшифровкаПлатежа;
	...
	НовыйДокППВх.СчетБанк = ПланыСчетов.Хозрасчетный.ТекущиеСчетаВНациональнойВалюте; //.НайтиПоКоду("3132")
	//
	...
	//
	НовыйДокППВх.СуммаДокумента  = Сумма;
	...
	НовыйДокППВх.СчетОрганизации = СчетОрганизации;
	//
	...
	НоваяРасшифровкаПлатежа.СуммаПлатежа  =  Сумма;
	НоваяРасшифровкаПлатежа.КратностьВзаиморасчетов  = 1;
	НоваяРасшифровкаПлатежа.СуммаВзаиморасчетов  =  Сумма;
	//
	НовыйДокППВх.Оплачено = Истина;
	НовыйДокППВх.Ответственный = ТекПользователь;
	
	НоваяРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств  = СтатьяДвиженияДенежныхСредств; 
	НоваяРасшифровкаПлатежа.СчетУчетаРасчетовСКонтрагентом  = СчетУчетаРасчетовСКонтрагентом;
	НовыйДокППВх.Комментарий = Комментарий;
...
Процедура ЧтениеИПроверкаДанных(Знач ТекущаяСтрока, N_кредита, ДатаОперации, ДоговорКонтрагента, Контрагент, Процент, Тело, ШтрафПеня, Сумма)
	
	ДатаОперации = ПреобразоватьВДату(ТекущаяСтрока.ДатаОперации+ " " + ТекущаяСтрока.ВремяОперации);
	N_кредита	= формат(ТекущаяСтрока.НомерКредита	,"ЧЦ=6; ЧГ=0");
	//
	Процент = 0+	формат(ТекущаяСтрока.Процент,"ЧЦ=7; ЧГ=2");
	Тело =	0+формат(ТекущаяСтрока.Тело,"ЧЦ=7; ЧГ=2");
	ШтрафПеня =	0+формат(ТекущаяСтрока.ШтрафПеня,"ЧЦ=7; ЧГ=2");
	Сумма =	0+формат(ТекущаяСтрока.Сумма,"ЧЦ=7; ЧГ=2");
	Если Сумма <> 0 И Сумма <>  ШтрафПеня+Тело+Процент Тогда
		ВызватьИсключение("Договор " +N_кредита+" не совпадает Сумма");
	КонецЕсли;		
...
Процедура СформироватьСтруктуруКолонок() Экспорт
	...
	Колонка = Новый Структура;
	Колонка.Вставить("НомерКолонки",НомерКолонки);
	Колонка.Вставить("ИмяРеквизита","ВремяОперации");
	Колонка.Вставить("ОписаниеТипов",  ОбщегоНазначения.ПолучитьОписаниеТиповДаты(ЧастиДаты.Время));
	//Колонка.Вставить("ШиринаКолонки",  10);
	Колонки.Вставить(Колонка.ИмяРеквизита,Колонка);
	НомерКолонки = НомерКолонки + 1;
	...
	Колонка = Новый Структура;
	Колонка.Вставить("НомерКолонки",НомерКолонки);
	Колонка.Вставить("ИмяРеквизита","НомерКредита");
	Колонка.Вставить("ОписаниеТипов",  ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(6,0));
	//Колонка.Вставить("ШиринаКолонки",  10);
	Колонки.Вставить(Колонка.ИмяРеквизита,Колонка);
	НомерКолонки = НомерКолонки + 1;
	...
	Колонка = Новый Структура;
	Колонка.Вставить("НомерКолонки",НомерКолонки);
	Колонка.Вставить("ИмяРеквизита","Тело");
	...
	Колонка = Новый Структура;
	Колонка.Вставить("НомерКолонки",НомерКолонки);
	Колонка.Вставить("ИмяРеквизита","Процент");
	Колонка.Вставить("ОписаниеТипов",  ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(7,2));
	//Колонка.Вставить("ШиринаКолонки",  10);
	Колонки.Вставить(Колонка.ИмяРеквизита,Колонка);
	НомерКолонки = НомерКолонки + 1;
	...
	Колонка = Новый Структура;
	Колонка.Вставить("НомерКолонки",НомерКолонки);
	Колонка.Вставить("ИмяРеквизита","ШтрафПеня");
	Колонка.Вставить("ОписаниеТипов",  ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(7,2));
	//Колонка.Вставить("ШиринаКолонки",  10);
	Колонки.Вставить(Колонка.ИмяРеквизита,Колонка);
	НомерКолонки = НомерКолонки + 1;
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	СчетОрганизации = Организация.ОсновнойБанковскийСчет ;  //Справочники.БанковскиеСчета.НайтиПоКоду("000000075" )
КонецПроцедуры
