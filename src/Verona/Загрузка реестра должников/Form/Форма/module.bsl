...
Функция ДобавитьСтрокуВ_БО(ТабличныйДокумент, ТекущаяСтрока)	
	...
	НомерДоговора	= формат(СтрЗаменить(ТекущаяСтрока.Операция, "приход от контрагента - погашения кредита и процентов, Кредит №", "")  ,"ЧЦ=5; ЧГ=0");// 
	...
	Попытка	
		...
		если ДоговорКонтрагента =  Справочники.ДоговорыКонтрагентов.ПустаяСсылка() Тогда
			...
			ВызватьИсключение("Не найден Договор " + НомерДоговора);
		КонецЕсли;		
		Контрагент  = ДоговорКонтрагента.Владелец;
		
		если Контрагент =  Справочники.Контрагенты.ПустаяСсылка() Тогда
			ВызватьИсключение("Договор " +НомерДоговора+" не имеет контрагента");
		КонецЕсли;	
		...
		ИначеЕсли  БухСчет = "373"  Тогда  
			...
			Если Число(НомерДоговора) >= 20515 Тогда //З договору № 20515: 3731
		...
		ИначеЕсли БухСчет = "374" Тогда                                                                                            
			НоваяРасшифровкаПлатежа.СтатьяДвиженияДенежныхСредств  = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("1113 Штраф за прострочення кредиту" );     
			Если Число(НомерДоговора) >= 20515 Тогда  //З договору № 20515: 
...
Функция СоздатьИЗаписатьНаборЗаписейПоСтроке(ТабличныйДокумент, ТекущаяСтрока, ДокОперация, РегХозрасчетныйСписаниеЗадолженности)	
	...
	Сумма =	формат(ТекущаяСтрока.Сумма,"ЧЦ=7; ЧГ=2");
		...
		если Контрагент =  Справочники.Контрагенты.ПустаяСсылка() Тогда
			...
			НовыйКонтрагент.Родитель = Справочники.Контрагенты.НайтиПоНаименованию("Боржники");
			...
			Иначе 
				...
				ДоговорКонтрагента = ПодготовитьИСоздатьДоговорКонтрагента(ДатаВыдачиКредита, Контрагент, НомерДоговора);
            	...
            	ЗаполнитьСвойствоДоговороаПоставщик(ДоговорКонтрагента);
				
				КонтрагентОсновнойДоговор(ДоговорКонтрагента, Контрагент);
		...
		Иначе	
			...
			если ДоговорКонтрагента =  Справочники.ДоговорыКонтрагентов.ПустаяСсылка() Тогда
				...
				КонтрагентОсновнойДоговор(ДоговорКонтрагента, Контрагент);
		Иначе	
			//		ВызватьИсключение("Договор с номером "+НомерДоговора+" уже существует");
		...
		РегЗапись.Содержание  = "Консолидация задолженности";
		...
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Контрагенты", Контрагент);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Договоры", ДоговорКонтрагента.Ссылка);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Контрагенты", Договор.Владелец);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Договоры", Договор);
...
Процедура КонтрагентОсновнойДоговор(Знач ДоговорКонтрагента, Знач Контрагент)
...
Процедура ЗаполнитьСвойствоДоговороаПоставщик(Знач ДоговорКонтрагента)
	
	Перем СтруктураСвойства;
	
	СтруктураСвойства = Новый Структура;
	СтруктураСвойства.Вставить("Свойство","Поставщик");
	СтруктураСвойства.Вставить("Значение", Договор.Владелец);
	ЗаписатьЗначенияСвойствА(ДоговорКонтрагента.Ссылка,СтруктураСвойства);

КонецПроцедуры

Функция ПодготовитьИСоздатьДоговорКонтрагента(Знач ДатаВыдачиКредита, Знач Контрагент, Знач НомерДоговора)
