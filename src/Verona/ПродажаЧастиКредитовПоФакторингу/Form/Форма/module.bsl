...
Функция СоздатьДокОперация()
	...
	ДокОперация.Содержание = "Продажа Факторинга";
	ДокОперация.Комментарий = "Продажа Факторинга";
...
Функция СоздатьИЗаписатьНаборЗаписейПоСтроке(ТабличныйДокумент, ТекущаяСтрока, ДокОперация, РегХозрасчетныйСписаниеЗадолженности)	
	...
	Сумма =	Число(?(ТекущаяСтрока.Сумма = "", "0", формат(ТекущаяСтрока.Сумма,"ЧЦ=7; ЧГ=2")));
	//Сумма=1;
	//Сумма37712 = Число(?(ТекущаяСтрока.Сумма37712 = "", "0", формат(ТекущаяСтрока.Сумма37712,"ЧЦ=7; ЧГ=2")));
	...
	Доход = Сумма-СуммаОстаток;
		...
		РегЗапись.Содержание  = "Продажа Факторинга";
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3771");
		РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("352");
    	БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Контрагенты", КонтрагентПокупатель);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Договоры", ДоговорКонтрагентаПокупателья);
    	...
    	БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Контрагенты", КонтрагентПродавец);
		...
		ДокОперация.СуммаОперации = ДокОперация.СуммаОперации + Сумма;
		ДокОперация.Записать();
	...
	Если Убыток>0 Тогда
		...
		РегЗапись.Регистратор = ДокОперация.Ссылка;
		РегЗапись.Организация = Организация;
		РегЗапись.Содержание  = "Убыток";
		РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("352");               
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("949");
		
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "ФинансовыеИнвестиции", Справочники.ФинансовыеИнвестиции.НайтиПоКоду("000000001"));
    	БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Контрагенты", КонтрагентПродавец);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Договоры", ДоговорКонтрагента);
		
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиЗатрат",СтатьяЗатрат);  
		РегЗапись.Сумма = Убыток;                                                                                           
		РегХозрасчетныйСписаниеЗадолженности.Записать();
		ДокОперация.СуммаОперации = ДокОперация.СуммаОперации + Убыток;
		ДокОперация.Записать();
	 ...
	 Иначе
    	...
    	БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Контрагенты", КонтрагентПродавец);
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Договоры", ДоговорКонтрагента);
		
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "СтатьиДоходов",СтатьяДоходов);  
...
Процедура СформироватьСтруктуруКолонок() Экспорт
	...
	//Колонка = Новый Структура;
	//Колонка.Вставить("НомерКолонки",НомерКолонки);
	//Колонка.Вставить("ИмяРеквизита","Датакредита");
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	СтатьяДоходов = 	Справочники.СтатьиДоходов.НайтиПоКоду("000001001");
