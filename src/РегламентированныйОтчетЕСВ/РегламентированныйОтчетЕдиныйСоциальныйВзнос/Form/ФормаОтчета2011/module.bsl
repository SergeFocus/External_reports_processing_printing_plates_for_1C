...
Процедура ВывестиНаПечатьЛист(СтруктураПараметровЛиста)
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица7" Тогда
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			ЗначениеВКлетку(СтрокаГруппы.Т7_Дней,      Ложь,    2, "Дней",         Строка, Истина);
			...
			ЗначениеВКлетку(СтрокаГруппы.Т7_Норма, Ложь,    6, "Норма",      Строка, Истина);
...
Функция ПолучитьСписокВидовРасчетовПоВидуНалога(Налог)
...
Процедура СформироватьНачисленияВзносыТаблица6(СписокРаботниковСоСпецСтажем)
	...
	//Тексты запросов для ЗУП, УПП и УТП
	//Для БП модифицируются ниже по коду
	...
	|	И (ВзносыВФонды.Результат <> 0 ИЛИ ВзносыВФонды.База <> 0 ИЛИ ВзносыВФонды.БазаВзноса <> 0)
	|ИНДЕКСИРОВАТЬ ПО
		...
		Иначе
			...
			Иначе
				...
				Если мМенеджерЗвит1С = Неопределено Тогда
					...
					Предупреждение(НСТР("ru = 'Проверьте правильность заполнения таблиц с видами расчетов по больничным!'; uk = 'Перевірте правильність заповнення таблиць з видами розрахунку лікарняних!'"));
				КонецЕсли;
