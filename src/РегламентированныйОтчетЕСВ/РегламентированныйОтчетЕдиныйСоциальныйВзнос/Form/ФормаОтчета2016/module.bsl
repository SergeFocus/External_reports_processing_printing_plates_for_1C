	...
	Для Каждого ЭлементСтруктуры Из мСтруктураМногострочныхРазделов Цикл
		...
		Для НСтр = 1 По СтруктураГруппы.Количество() Цикл
			...
			Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
				...
				// А_Вячеслав ( для открытия старых версий отчета
				Если Найти(ИмяЯчейки,"T1R")>0 И Найти(ИмяЯчейки,"G111")>0 И Найти(ИмяЯчейки,"G111D")=0 Тогда
						ИмяЯчейки = СтрЗаменить(ИмяЯчейки,"G111","G111D");
				КонецЕсли;	
				Если Найти(ИмяЯчейки,"T1R")>0 И Найти(ИмяЯчейки,"G112")>0 И Найти(ИмяЯчейки,"G112D")=0 Тогда
					Продолжить;
				КонецЕсли;	
				// А_Вячеслав ) 		
...
Процедура ВывестиРазделВТабличныйДокумент(ИмяГруппы, СтруктураГруппы)
	...
	Если ТекущееКоличествоСтрокГруппы > ИсходноеКоличествоСтрокГруппы Тогда
		...
		Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
			...
			// А_Вячеслав ( для открытия старых версий отчета
			Если Найти(ИмяОбластиЯчейки,"T1R")>0 И Найти(ИмяОбластиЯчейки,"G111")>0 И Найти(ИмяОбластиЯчейки,"G111D")=0 Тогда
					ИмяОбластиЯчейки = СтрЗаменить(ИмяОбластиЯчейки,"G111","G111D");
			КонецЕсли;	
			Если Найти(ИмяОбластиЯчейки,"T1R")>0 И Найти(ИмяОбластиЯчейки,"G112")>0 И Найти(ИмяОбластиЯчейки,"G112D")=0 Тогда
				Продолжить;
			КонецЕсли;	
			// А_Вячеслав ) 
		...
		Для НомерСтроки = 1 По ТекущееКоличествоСтрокГруппы Цикл
			...
			Для Каждого Колонка Из СтруктураГруппы.Колонки Цикл
				...
				Если Найти(ИмяОбластиЯчейки,"T1R")>0 И Найти(ИмяОбластиЯчейки,"G111")>0 И Найти(ИмяОбластиЯчейки,"G111D")=0 Тогда
					ИмяОбластиЯчейки = СтрЗаменить(ИмяОбластиЯчейки,"G111","G111D");
				КонецЕсли;	
				Если (Найти(ИмяОбластиЯчейки,"T1R")>0 И Найти(ИмяОбластиЯчейки,"G112")>0 И Найти(ИмяОбластиЯчейки,"G112D")=0) ИЛИ ИмяКолонки = "T1RG112" Тогда
					Продолжить;
				КонецЕсли;	
				// А_Вячеслав )
	...
	Для Каждого СтрокаТаблицы Из СтруктураГруппы Цикл
		Для Каждого КолонкаТаблицы Из СтруктураГруппы.Колонки Цикл
			...
			Если ОбластьЯчейкиТаблицы.СодержитЗначение Тогда
				...
				// А_Вячеслав (  для открытия старых версий отчета
				Если лБылаЗамена Тогда
					Попытка
						ОбластьЯчейкиТаблицы.Значение = Дата(2016,число(ЗначениеЯчейки),1);	
					Исключение
...
Процедура ПриОткрытии()
	...
	МассивПанелей.Добавить("ПереченьТаблиц");
	МассивПанелей.Добавить("Таблица1");
	МассивПанелей.Добавить("Таблица5");
	МассивПанелей.Добавить("Таблица6");
	МассивПанелей.Добавить("Таблица7");
...
Процедура ВыгрузитьЛистВDBF(СтруктураПараметровЛиста)
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица5" Тогда
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			докЭкспорт.UKR_GROMAD 	= СтрокаГруппы.T1RG6;
			
			докЭкспорт.ZO 	= СтрокаГруппы.T1RG7;
			
			докЭкспорт.DOG_CPH 	= СтрокаГруппы.T1RG8;
			
			докЭкспорт.NUMIDENT = СтрокаГруппы.T1RG9S;
			докЭкспорт.LN		= СтрокаГруппы.T1RG101S;
			докЭкспорт.NM		= СтрокаГруппы.T1RG102S;
			докЭкспорт.FTN		= СтрокаГруппы.T1RG103S;
			докЭкспорт.START_DT = ?(ЗначениеЗаполнено(СтрокаГруппы.T1RG111D), День(СтрокаГруппы.T1RG111D),"");
			докЭкспорт.END_DT   = ?(ЗначениеЗаполнено(СтрокаГруппы.T1RG112D), День(СтрокаГруппы.T1RG112D),"");
			
			докЭкспорт.NRM_DT	= ?(ЗначениеЗаполнено(СтрокаГруппы.T1RG12D), Формат(СтрокаГруппы.T1RG12D, "ДФ=ddMMyyyy"),"");
			
			докЭкспорт.PID_ZV		= СтрокаГруппы.T1RG13S;
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица6" Тогда
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			докЭкспорт.ROWNUM 		= КоличествоСтрок;
			докЭкспорт.UKR_GROMAD 	= СтрокаГруппы.T1RG6;
			докЭкспорт.ST		 	= ?(ВРЕГ(СтрокаГруппы.T1RG7) = "Ч", 1, 0);
			
			докЭкспорт.NUMIDENT = СтрокаГруппы.T1RG8S;
			докЭкспорт.LN		= СтрокаГруппы.T1RG121S;
			докЭкспорт.NM		= СтрокаГруппы.T1RG122S;
			докЭкспорт.FTN		= СтрокаГруппы.T1RG123S;
			докЭкспорт.ZO		= СтрокаГруппы.T1RG9;
			докЭкспорт.PAY_TP		= СтрокаГруппы.T1RG10;
			
			// А_Вячеслав ( для открытия старых версий отчета
			//Если ЗначениеЗаполнено(СтрокаГруппы.T1RG111D) Тогда
			//	докЭкспорт.PAY_MNTH 	= Месяц(СтрокаГруппы.T1RG111D);
			//	докЭкспорт.PAY_YEAR 	= Год(СтрокаГруппы.T1RG111D);
			Если СтруктураГруппы.Колонки.Найти("T1RG111D")<>Неопределено И ЗначениеЗаполнено(СтрокаГруппы.T1RG111D) Тогда	
				докЭкспорт.PAY_MNTH 	= Месяц(СтрокаГруппы.T1RG111D);
				докЭкспорт.PAY_YEAR 	= Год(СтрокаГруппы.T1RG111D);
			ИначеЕсли СтруктураГруппы.Колонки.Найти("T1RG111")<>Неопределено И ЗначениеЗаполнено(СтрокаГруппы.T1RG111) Тогда		
				докЭкспорт.PAY_MNTH 	= СтрокаГруппы.T1RG111;
				докЭкспорт.PAY_YEAR 	= СтрокаГруппы.T1RG112;
			// А_Вячеслав ) 		
			КонецЕсли;
			
			докЭкспорт.KD_NP		= СтрокаГруппы.T1RG13;
			докЭкспорт.KD_NZP		= СтрокаГруппы.T1RG14;
			
			докЭкспорт.KD_PTV	= СтрокаГруппы.T1RG15;
			докЭкспорт.KD_VP	= СтрокаГруппы.T1RG16;
			
			докЭкспорт.SUM_TOTAL= СтрокаГруппы.T1RG17;
			докЭкспорт.SUM_MAX	= СтрокаГруппы.T1RG18;
			докЭкспорт.SUM_DIFF	= СтрокаГруппы.T1RG19;
			докЭкспорт.SUM_INS  = СтрокаГруппы.T1RG20;
			докЭкспорт.SUM_NARAH= СтрокаГруппы.T1RG21;
			докЭкспорт.OTK		= СтрокаГруппы.T1RG22;
			докЭкспорт.EXP		= СтрокаГруппы.T1RG23;
			
			докЭкспорт.NRC		= СтрокаГруппы.T1RG24;
			докЭкспорт.NRM		= СтрокаГруппы.T1RG25;
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица7" Тогда
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			докЭкспорт.NUMIDENT = СтрокаГруппы.T1RG7S;
			докЭкспорт.LN		= СтрокаГруппы.T1RG91S;
			докЭкспорт.NM		= СтрокаГруппы.T1RG92S;
			докЭкспорт.FTN		= СтрокаГруппы.T1RG93S;
			докЭкспорт.C_PID	= СтрокаГруппы.T1RG8S;
			докЭкспорт.DAYS		= СтрокаГруппы.T1RG12;
			// А_Вячеслав ( 12.12.2016  
			//докЭкспорт.START_DT	= СтрокаГруппы.T1RG10;
			//докЭкспорт.STOP_DT	= СтрокаГруппы.T1RG11;
			докЭкспорт.START_DT	= ?(ТипЗнч(СтрокаГруппы.T1RG10)=Тип("Дата"),День(СтрокаГруппы.T1RG10),СтрокаГруппы.T1RG10);
			докЭкспорт.STOP_DT	= ?(ТипЗнч(СтрокаГруппы.T1RG11)=Тип("Дата"),День(СтрокаГруппы.T1RG11),СтрокаГруппы.T1RG11);
			// А_Вячеслав) 
			
			докЭкспорт.HH		= СтрокаГруппы.T1RG131;
			докЭкспорт.MM		= СтрокаГруппы.T1RG132;
			докЭкспорт.SEAZON	= СтрокаГруппы.T1RG17;
			докЭкспорт.NORMA_1	= СтрокаГруппы.T1RG141;
			докЭкспорт.NORMA_2	= СтрокаГруппы.T1RG142;
			докЭкспорт.NORMA_3	= СтрокаГруппы.T1RG143;
			
			докЭкспорт.NUM_NAK	= СтрокаГруппы.T1RG15S;
			Попытка
			докЭкспорт.DT_NAK	= Число(Формат(СтрокаГруппы.T1RG16D, "ДФ=ddMMyyyy"));
			Исключение
			КонецПопытки;
...
Процедура КоманднаяПанельДействийXML(Кнопка) Экспорт
	...
	//**********************************************************************
	//Доработаем мСтруктураМногострочныхРазделов, 
	//Добавим 111 и 112 таблицы 5 с типом чило заполним их из 111D и 112D
	//Добавим 111 и 112 таблицы 6 с типом чило заполним их из 111D
	ОписаниеТиповЧисло0 = ОбщегоНазначения.ПолучитьОписаниеТиповЧисла(10,0);
	тмСтруктураМногострочныхРазделов = Новый Структура;
	тмСтруктураМногострочныхРазделов.Вставить("T5T1R", мСтруктураМногострочныхРазделов.T5T1R.Скопировать());
	тмСтруктураМногострочныхРазделов.Вставить("T6T1R", мСтруктураМногострочныхРазделов.T6T1R.Скопировать());
	тмСтруктураМногострочныхРазделов.Вставить("T7T1R", мСтруктураМногострочныхРазделов.T7T1R.Скопировать());
	// А_Вячеслав (  для открытия старых версий отчета
	Если тмСтруктураМногострочныхРазделов.T5T1R.Колонки.Найти("T1RG111")=Неопределено Тогда
	// А_Вячеслав ) 	
	тмСтруктураМногострочныхРазделов.T5T1R.Колонки.Добавить("T1RG111",ОписаниеТиповЧисло0);
	тмСтруктураМногострочныхРазделов.T5T1R.Колонки.Добавить("T1RG112",ОписаниеТиповЧисло0);
	тмСтруктураМногострочныхРазделов.T6T1R.Колонки.Добавить("T1RG111",ОписаниеТиповЧисло0);
	тмСтруктураМногострочныхРазделов.T6T1R.Колонки.Добавить("T1RG112",ОписаниеТиповЧисло0);
	// А_Вячеслав (  для открытия старых версий отчета 
	КонецЕсли;
	Если тмСтруктураМногострочныхРазделов.T5T1R.Колонки.Найти("T1RG111D")<>Неопределено Тогда
    // А_Вячеслав ) 
	Для Каждого СтрокаТаб Из тмСтруктураМногострочныхРазделов.T5T1R Цикл
		Если ЗначениеЗаполнено(СтрокаТаб["T1RG111D"]) Тогда
			СтрокаТаб["T1RG111"] = День(СтрокаТаб["T1RG111D"]);
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаб["T1RG112D"]) Тогда
			СтрокаТаб["T1RG112"] = День(СтрокаТаб["T1RG112D"]);
		КонецЕсли;
	КонецЦикла;
	Для Каждого СтрокаТаб Из тмСтруктураМногострочныхРазделов.T6T1R Цикл
		Если ЗначениеЗаполнено(СтрокаТаб["T1RG111D"]) Тогда
			СтрокаТаб["T1RG111"] = Месяц(СтрокаТаб["T1RG111D"]);
			СтрокаТаб["T1RG112"] = Год(СтрокаТаб["T1RG111D"]);
		КонецЕсли;
	КонецЦикла;
	// А_Вячеслав (  для открытия старых версий отчета
	КонецЕсли;

	// Т.к. в отчете колонки с типом дата, то при выгрузке превращаем их в число (через одно место)
	тмСтруктураМногострочныхРазделов.T7T1R.Колонки.Добавить("T1RG10__INT",ОписаниеТиповЧисло0);
	тмСтруктураМногострочныхРазделов.T7T1R.Колонки.Добавить("T1RG11__INT",ОписаниеТиповЧисло0);
	Для Каждого СтрокаТаб Из тмСтруктураМногострочныхРазделов.T7T1R Цикл
		Если ЗначениеЗаполнено(СтрокаТаб["T1RG10"]) Тогда
			СтрокаТаб["T1RG10__INT"] = День(СтрокаТаб["T1RG10"]);
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаб["T1RG11"]) Тогда
			СтрокаТаб["T1RG11__INT"] = День(СтрокаТаб["T1RG11"]);
		КонецЕсли;
	КонецЦикла;
	тмСтруктураМногострочныхРазделов.T7T1R.Колонки.Удалить("T1RG10");
	тмСтруктураМногострочныхРазделов.T7T1R.Колонки.Удалить("T1RG11");
	тмСтруктураМногострочныхРазделов.T7T1R.Колонки.Добавить("T1RG10",ОписаниеТиповЧисло0);
	тмСтруктураМногострочныхРазделов.T7T1R.Колонки.Добавить("T1RG11",ОписаниеТиповЧисло0);
	Для Каждого СтрокаТаб Из тмСтруктураМногострочныхРазделов.T7T1R Цикл
		Если ЗначениеЗаполнено(СтрокаТаб["T1RG10__INT"]) Тогда
			СтрокаТаб["T1RG10"] = СтрокаТаб["T1RG10__INT"];
		КонецЕсли;
		Если ЗначениеЗаполнено(СтрокаТаб["T1RG11__INT"]) Тогда
			СтрокаТаб["T1RG11"] = СтрокаТаб["T1RG11__INT"];
		КонецЕсли;
	КонецЦикла;
    // А_Вячеслав ) 
	//**********************************************************************
	...
	// вставляем данные многострочных разделов
	спПараметры.Вставить("ДанныеМногострочныхРазделов", тмСтруктураМногострочныхРазделов);
	...
	ФормаОбр.ИмяНужногоОтчета = "РегламентированныйОтчетЕдиныйСоциальныйВзнос";
...
Функция ПолучитьНастройкиПроверкиЗаполенения(Макет = "СоставПоказателей2016") Экспорт
	...
	Для Инд = 0 По МакетСоставаПоказателей.Области.Количество() - 1 Цикл
		...
		// если не выгружаем приложение, то зачем его проверять?
		Если тбСхемыXML.Найти(ИмяСтраницы, "НазваниеСтраницыПанели") = Неопределено Тогда
			Продолжить;
		КонецЕсли;
...
Процедура СоздатьТаблицуСоСхемамиВыгрузки() Экспорт
	...
	// А_Вячеслав (добавил новые схемы
	Если мДатаКонцаПериодаОтчета >= Дата(2016,11,01) Тогда
	Если Организация.ЮрФизЛицо = Перечисления.ЮрФизЛицо.ЮрЛицо Тогда
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "J3000411"; 
	...
	Иначе // физ. лицо	
		...
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F3000411"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Отчет по единому взносу';uk='Звіт по єдиному внеску'");
		НовСтрока.НазваниеСтраницыПанели ="ПереченьТаблиц";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F3040111"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Таблица 1';uk='Таблиця 1'");
		НовСтрока.НазваниеСтраницыПанели ="Таблица1";
				
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F3040511";
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Таблица 5';uk='Таблиця 5'");
		НовСтрока.НазваниеСтраницыПанели ="Таблица5";
		НовСтрока.Префикс = "T5";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F3040611";
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Таблица 6';uk='Таблиця 6'");
		НовСтрока.НазваниеСтраницыПанели ="Таблица6";
		НовСтрока.Префикс = "T6";
		
		НовСтрока = тбСхемыXML.Добавить();
		НовСтрока.ИмяСхемы = "F3040711"; 
		НовСтрока.ПредставлениеСхемы =НСтр("ru='Таблица 7';uk='Таблиця 7'");
		НовСтрока.НазваниеСтраницыПанели ="Таблица7";
		НовСтрока.Префикс = "T7";
...
Процедура ПроверитьЗаполнениеОсобыхПоказателей(ТаблицаНезаполненныхПоказателей,ДанныеОтчета)
	...
	Для Каждого ИмяПанели из МассивПанелей Цикл
		...
		ПоказательЕстьНаПанели = ДанныеОтчета["ПолеТабличногоДокумента"+ИмяПанели].Свойство(ИмяПоказателя);
		
		Если ПоказательЕстьНаПанели = Истина Тогда
...
//СтруктураГруппы7.Колонки.Добавить("T1RG10",   	 ОписаниеТиповЧисло2);
//СтруктураГруппы7.Колонки.Добавить("T1RG11",   	 ОписаниеТиповЧисло2); 
// А_Вячеслав) 
