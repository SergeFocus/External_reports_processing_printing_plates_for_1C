...
Процедура ЗаполнитьСведенияОбОрганизации()
	...
	Если Результат <> Неопределено Тогда
		...
		мПереченьТаблиц.Области.РуководительФИО.Значение = Лев(СокрЛ(ФИОСрез.Имя), 1) + ". " + Лев(СокрЛ(ФИОСрез.Отчество), 1) + ". " + СокрЛП(ФИОСрез.Фамилия);
	КонецЕсли;
	...
	Если Результат <> Неопределено Тогда
		ДанныеФИО = Новый Структура("ФизЛицо",Результат.ФизическоеЛицо);
		ФИОСрез = РегистрыСведений.ФИОФизЛиц.ПолучитьПоследнее(ДатаПодписи, ДанныеФИО);
		мПереченьТаблиц.Области.БухгалтерФИО.Значение = Лев(СокрЛ(ФИОСрез.Имя), 1) + ". " + Лев(СокрЛ(ФИОСрез.Отчество), 1) + ". " + СокрЛП(ФИОСрез.Фамилия);
	КонецЕсли;
	...
	мТаблица1.Области.КоэффициентДа.Значение   = "";
	мТаблица1.Области.КоэффициентНЕТ.Значение  = "X";
	Коэффициент = 0;
	
	РегКоэфф = РегистрыСведений.КоэффициентыСтавкиВзносовФОТ.СоздатьНаборЗаписей();
	РегКоэфф.Отбор.Организация.Значение = Организация;
	РегКоэфф.Отбор.Организация.Использование = Истина;
	РегКоэфф.Отбор.МесяцНачисления.Значение = НачалоМесяца(мДатаНачалаПериодаОтчета);
	РегКоэфф.Отбор.МесяцНачисления.Использование = Истина;
	РегКоэфф.Прочитать();
	Если РегКоэфф.Количество() = 0 Тогда
		Коэффициент = 0;	
	Иначе
		Коэффициент	= РегКоэфф[0].КоэффициентСтавки;
		Если Коэффициент > 0.999
			ИЛИ Коэффициент < 0.4 Тогда
			Коэффициент = 0;	
		КонецЕсли;
	КонецЕсли;
	
	мСлужебныеДанные.Области.Коэффициент.Значение = Коэффициент;
	
	Если Коэффициент > 0 Тогда
		
		мТаблица1.Области.КоэффициентДа.Значение   = "Х";
		мТаблица1.Области.КоэффициентНЕТ.Значение  = "";
		
		мТаблица1.Области.СредняяЗП2014.Значение   = РегКоэфф[0].База2014;
		Попытка
			мТаблица1.Области.СреднееЗЛ2014.Значение   = РегКоэфф[0].КоличествоЗЛ2014;
		Исключение
		КонецПопытки;
...
Процедура ВывестиНаПечатьЛист(СтруктураПараметровЛиста)
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица5" Тогда
		...
		Макет = ПолучитьМакет("Таблица5_2015");
		
		Шапка  = Макет.ПолучитьОбласть("Шапка");
		ЗначениеВКлетку(мПереченьТаблиц.Области.ЕДРПОУ.Значение + "          ", Ложь, 10, "ЕДРПОУ", Шапка, Истина);
		ЗначениеВКлетку(мПереченьТаблиц.Области.ЛиквЕДРПОУ.Значение + "     ", Ложь, 10, "ЛиквЕДРПОУ", Шапка, Истина);
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица6" Тогда
		...
		Макет = ПолучитьМакет("Таблица6_2015");
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			ЗначениеВКлетку(ПолучитьГривны(СтрокаГруппы.Т6_СуммаРазницы), Ложь,    6, "СуммаРазницыГрн",    Строка, Истина);			
			ЗначениеВКлетку(ПолучитьКопейки(СтрокаГруппы.Т6_СуммаРазницы), Истина,   2, "СуммаРазницыКоп",    Строка, Истина);			
			
			ЗначениеВКлетку(ПолучитьГривны(СтрокаГруппы.Т6_СуммаВзносовУдержано), Ложь,    6, "СуммаВзносовУдержаноГрн",    Строка, Истина);			
			ЗначениеВКлетку(ПолучитьКопейки(СтрокаГруппы.Т6_СуммаВзносовУдержано), Истина,   2, "СуммаВзносовУдержаноКоп",    Строка, Истина);			
			
			ЗначениеВКлетку(ПолучитьГривны(СтрокаГруппы.Т6_СуммаВзносовНачислено), Ложь,    6, "СуммаВзносовНачисленоГрн",    Строка, Истина);			
			ЗначениеВКлетку(ПолучитьКопейки(СтрокаГруппы.Т6_СуммаВзносовНачислено), Истина,   2, "СуммаВзносовНачисленоКоп",    Строка, Истина);			
			...
			ИтогоРазницы 			= ИтогоРазницы 			 + СтрокаГруппы.Т6_СуммаРазницы;
			ИтогоВзносыУдержано		= ИтогоВзносыУдержано	 + СтрокаГруппы.Т6_СуммаВзносовУдержано;
			ИтогоВзносыНачислено	= ИтогоВзносыНачислено	 + СтрокаГруппы.Т6_СуммаВзносовНачислено;
			...
			Строка.Параметры.НеполноеРВ  = Формат(СтрокаГруппы.Т6_НеполноеРВ,"ЧЦ=1; ЧН=");
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица7" Тогда
		...
		Макет = ПолучитьМакет("Таблица7_2015");
...
Процедура Расчет()
	...
	мТаблица1.Области.П2_5.Значение = ОкруглитьЧислоПоФормату(мТаблица1.Области.П2_5_1.Значение) 
								    + ОкруглитьЧислоПоФормату(мТаблица1.Области.П2_5_2.Значение)
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П2_5_3.Значение)
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П2_5_4.Значение)
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П2_5_5.Значение);
	...
	мТаблица1.Области.П2.Значение   = ОкруглитьЧислоПоФормату(мТаблица1.Области.П2_1.Значение) 
									...
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П2_5.Значение);
	...
	мТаблица1.Области.П3_5.Значение = ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_5_1.Значение) 
								    + ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_5_2.Значение)
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_5_3.Значение)
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_5_4.Значение)
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_5_5.Значение);
	...
	мТаблица1.Области.П3.Значение   = ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_1.Значение) 
									...
									+ ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_5.Значение);
	...
	мТаблица1.Области.П6_1.Значение   = ОкруглитьЧислоПоФормату(мТаблица1.Области.П6_1_1.Значение) 
									  ...
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П6_1_9.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П6_1_10.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П6_1_11.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П6_1_12.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П6_1_13.Значение)
									  ;
	...
	мТаблица1.Области.П7_1.Значение   = ОкруглитьЧислоПоФормату(мТаблица1.Области.П7_1_1.Значение) 
									  ...
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П7_1_9.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П7_1_10.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П7_1_11.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П7_1_12.Значение)
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П7_1_13.Значение)
									  ;
	...
	мТаблица1.Области.П8_1.Значение   = ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_1_1.Значение) 
									  + ОкруглитьЧислоПоФормату(мТаблица1.Области.П3_5_1.Значение)
	...
	мТаблица6.Области.СуммаРазницыВсего.Значение 			= СтруктураГруппы6.Итог("Т6_СуммаРазницы");
	мТаблица6.Области.СуммаВзносовУдержаноВсего.Значение 	= СтруктураГруппы6.Итог("Т6_СуммаВзносовУдержано");
	мТаблица6.Области.СуммаВзносовНачисленоВсего.Значение 	= СтруктураГруппы6.Итог("Т6_СуммаВзносовНачислено");
...
Процедура РассчитатьСуммуФактическихЗатратНаОплатуТруда()
	...
	СЗ.Добавить(Справочники.Налоги.ЕСВМобилизованные);
...
Процедура ЗаполнитьАвто()  Экспорт
	...
	Исключение
		Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	СУММА(ВзносыВФондыИНалоги.БазаВзноса) 				 КАК База,
	               |	СУММА(ВзносыВФондыИНалоги.Результат)  				 КАК СуммаВзноса,
				   |	СУММА(0)			 								 КАК БазаДоначисление,
				   |	СУММА(0)	 										 КАК СуммаВзносаДоначисление
				   |ИЗ
	               |	РегистрРасчета."+ИмяРегистра+" КАК ВзносыВФондыИНалоги
				   |
				   |ГДЕ
	...
	П2_5_1  = 0;    П2_5_2	= 0;	П2_5_3	= 0;	П2_5_4	= 0;	П2_5_5	= 0;
	
	П3_3_1  = 0;    П3_3_2	= 0;	П3_3_3	= 0;    П3_3_4	= 0;
	П3_4_1  = 0;    П3_4_2	= 0;	П3_4_3	= 0;    П3_4_4	= 0;
	
	П3_5_1  = 0;    П3_5_2	= 0;	П3_5_3	= 0;    П3_5_4	= 0;    П3_5_5	= 0;
		...
		Для каждого СтрокаБаз Из ТаблицаБазВзносов Цикл
			...
			Если СтрокаБаз.СтатьяНалоговойДекларации = СтрокиДекларации.ЕСВ_ФОТ Тогда
				Если СтрокаБаз.ТипНачисления < 2 Тогда
					Если СтрокаБаз.БазаДоначисление = 0 Тогда
						База = СтрокаБаз.База;
					Иначе
						База = СтрокаБаз.База - СтрокаБаз.БазаДоначисление;
						П2_5_1 = П2_5_1 + СтрокаБаз.БазаДоначисление;
					КонецЕсли;	
					Если СтрокаБаз.СуммаВзносаДоначисление = 0 Тогда
						СуммаВзноса = СтрокаБаз.СуммаВзноса;
					Иначе
						СуммаВзноса = СтрокаБаз.СуммаВзноса - СтрокаБаз.СуммаВзносаДоначисление;
						П3_5_1 = П3_5_1 + СтрокаБаз.СуммаВзносаДоначисление;
					КонецЕсли;
					П2_1_1 = П2_1_1 + База;
					П3_1_1 = П3_1_1 + СуммаВзноса;
			...
			Если СтрокаБаз.СтатьяНалоговойДекларации = СтрокиДекларации.ЕСВ_ФОТБюджетники Тогда
				Если СтрокаБаз.ТипНачисления < 2 Тогда
					...
					Иначе
						База = СтрокаБаз.База - СтрокаБаз.БазаДоначисление;
						П2_5_2 = П2_5_2 + СтрокаБаз.БазаДоначисление;
					КонецЕсли;	
					Если СтрокаБаз.СуммаВзносаДоначисление = 0 Тогда
						СуммаВзноса = СтрокаБаз.СуммаВзноса;
					Иначе
						СуммаВзноса = СтрокаБаз.СуммаВзноса - СтрокаБаз.СуммаВзносаДоначисление;
						П3_5_2 = П3_5_2 + СтрокаБаз.СуммаВзносаДоначисление;
					КонецЕсли;
					П2_1_2 = П2_1_2 + База;
					П3_1_2 = П3_1_2 + СуммаВзноса;
				ИначеЕсли СтрокаБаз.ТипНачисления = 2 ИЛИ СтрокаБаз.ТипНачисления = 4 Тогда
					П6_4 = П6_4 + СтрокаБаз.База;
			...
			Если СтрокаБаз.СтатьяНалоговойДекларации = СтрокиДекларации.ЕСВ_ФОТЛетчики Тогда
				Если СтрокаБаз.ТипНачисления < 2 Тогда
					...
					Иначе
						База = СтрокаБаз.База - СтрокаБаз.БазаДоначисление;
						П2_5_3 = П2_5_3 + СтрокаБаз.БазаДоначисление;
					КонецЕсли;	
					Если СтрокаБаз.СуммаВзносаДоначисление = 0 Тогда
						СуммаВзноса = СтрокаБаз.СуммаВзноса;
					Иначе
						СуммаВзноса = СтрокаБаз.СуммаВзноса - СтрокаБаз.СуммаВзносаДоначисление;
						П3_5_3 = П3_5_3 + СтрокаБаз.СуммаВзносаДоначисление;
					КонецЕсли;
					П2_1_3 = П2_1_3 + База;
					П3_1_3 = П3_1_3 + СуммаВзноса;
				ИначеЕсли СтрокаБаз.ТипНачисления = 2 ИЛИ СтрокаБаз.ТипНачисления = 4 Тогда
					П6_4 = П6_4 + СтрокаБаз.База;
			...
			Попытка
				Если СтрокаБаз.СтатьяНалоговойДекларации = СтрокиДекларации.ЕСВ_ФОТСудостр Тогда
					Если СтрокаБаз.ТипНачисления < 2 Тогда
						...
						Иначе
							База = СтрокаБаз.База - СтрокаБаз.БазаДоначисление;
							П2_5_4 = П2_5_4 + СтрокаБаз.БазаДоначисление;
						КонецЕсли;	
						Если СтрокаБаз.СуммаВзносаДоначисление = 0 Тогда
							СуммаВзноса = СтрокаБаз.СуммаВзноса;
						Иначе
							СуммаВзноса = СтрокаБаз.СуммаВзноса - СтрокаБаз.СуммаВзносаДоначисление;
							П3_5_4 = П3_5_4 + СтрокаБаз.СуммаВзносаДоначисление;
						КонецЕсли;
			...
			Если СтрокаБаз.СтатьяНалоговойДекларации = СтрокиДекларации.ЕСВ_ФОТБольничные Тогда
				Если СтрокаБаз.ТипНачисления < 2 Тогда
					...
					Иначе
						База = СтрокаБаз.База - СтрокаБаз.БазаДоначисление;
						П2_5_5 = П2_5_5 + СтрокаБаз.БазаДоначисление;
					КонецЕсли;	
					Если СтрокаБаз.СуммаВзносаДоначисление = 0 Тогда
						СуммаВзноса = СтрокаБаз.СуммаВзноса;
					Иначе
						СуммаВзноса = СтрокаБаз.СуммаВзноса - СтрокаБаз.СуммаВзносаДоначисление;
						П3_5_5 = П3_5_5 + СтрокаБаз.СуммаВзносаДоначисление;
					КонецЕсли;
					П2_3_1 = П2_3_1 + База;
					П3_3_1 = П3_3_1 + СуммаВзноса;
				ИначеЕсли СтрокаБаз.ТипНачисления = 2 ИЛИ СтрокаБаз.ТипНачисления = 4 Тогда
					П6_4 = П6_4 + СтрокаБаз.База;
					П6_1_8 = П6_1_8 + СтрокаБаз.СуммаВзноса;
				ИначеЕсли СтрокаБаз.ТипНачисления = 3 ИЛИ СтрокаБаз.ТипНачисления = 5 Тогда
    ...
    УстановитьЗначениеПоказателя("П2_5_1", 	П2_5_1);УстановитьЗначениеПоказателя("П2_5_2", П2_5_2);УстановитьЗначениеПоказателя("П2_5_3", П2_5_3);УстановитьЗначениеПоказателя("П2_5_4", П2_5_4);УстановитьЗначениеПоказателя("П2_5_5", П2_5_5);	
    ...
    УстановитьЗначениеПоказателя("П3_5_1", 	П3_5_1);УстановитьЗначениеПоказателя("П3_5_2", П3_5_2);УстановитьЗначениеПоказателя("П3_5_3", П3_5_3);УстановитьЗначениеПоказателя("П3_5_4", П3_5_4);УстановитьЗначениеПоказателя("П3_5_5", П3_5_5);	
	...
	// согласно Лист ДФС України від 28.01.2016 №2844/7/99-99-17-03-01-17 "Про надання роз‘яснення"
	Если мДатаКонцаПериодаОтчета > '2016-01-01' Тогда
		УстановитьЗначениеПоказателя("П4_1_1", 	0);
		УстановитьЗначениеПоказателя("П4_1_2", 	0);
		УстановитьЗначениеПоказателя("П4_1_3", 	0);
		УстановитьЗначениеПоказателя("П4_2", 	0);
		УстановитьЗначениеПоказателя("П4_3", 	0);
		УстановитьЗначениеПоказателя("П4_4", 	0);
		
		УстановитьЗначениеПоказателя("П5_1", 	0);
		УстановитьЗначениеПоказателя("П5_2", 	0);
		УстановитьЗначениеПоказателя("П5_3", 	0);
		УстановитьЗначениеПоказателя("П5_4", 	0);
		УстановитьЗначениеПоказателя("П5_5", 	0);
		УстановитьЗначениеПоказателя("П5_6", 	0);
	КонецЕсли;
...
Процедура СформироватьНачисленияВзносыТаблица6(СписокРаботниковСоСпецСтажем)
	...
	|	ВзносыВФонды.ВидРасчета.КатегорияРасчета КАК КатегорияРасчета,
	|	НАЧАЛОПЕРИОДА(ВзносыВФонды.БазовыйПериодНачало,МЕСЯЦ) КАК Период,
	|	СУММА(0) КАК ДоначислениеФОТ,
	|	СУММА(ВзносыВФонды.Результат) КАК НалогФОТ,
	|	СУММА(0) КАК ДоначислениеНалогФОТ
	...
	| Сотрудник, Период
	|";
	ТекстЗапросаВТФОТ_01052015 ="ВЫБРАТЬ РАЗРЕШЕННЫЕ
	|	ВзносыВФонды.Сотрудник,
	|	ВзносыВФонды.ВидРасчета.КатегорияРасчета КАК КатегорияРасчета,
	|	НАЧАЛОПЕРИОДА(ВзносыВФонды.БазовыйПериодНачало,МЕСЯЦ) КАК Период,
	|	СУММА(ВзносыВФонды.БазаДоначисление) КАК ДоначислениеФОТ,
	|	СУММА(ВзносыВФонды.Результат) КАК НалогФОТ,
	|	СУММА(ВзносыВФонды.РезультатДоначисление) КАК ДоначислениеНалогФОТ
	|ПОМЕСТИТЬ ВТВзносыФОТ
	|ИЗ
	|	РегистрРасчета.ВзносыВФонды КАК ВзносыВФонды
	|ГДЕ
	...
	|	ВзносыВФонды.ФизЛицо.КодПоДРФО КАК КодПоДРФО,
	|	ВЫБОР 
	|	КОГДА ЕстьNULL(ГражданствоФизЛиц.НеЯвляетсяНалоговымРезидентом, ЛОЖЬ)
	|		ТОГДА 0
	|	КОГДА ЕстьNULL(ГражданствоФизЛиц.Страна, ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.ПустаяСсылка)) = ЗНАЧЕНИЕ(Справочник.КлассификаторСтранМира.ПустаяСсылка) 
	...
	|   ВзносыВФонды.Налог,
	|   ЕСТЬNULL(ВзносыФОТ.ДоначислениеФОТ, 0) КАК ДоначислениеФОТ,
	|   ЕСТЬNULL(ВзносыФОТ.НалогФОТ, 0) КАК НалогФОТ,
	|   ЕСТЬNULL(ВзносыФОТ.ДоначислениеНалогФОТ, 0) КАК ДоначислениеНалогФОТ,
	...
	|    И ВзносыВФонды.КодКатегорииЗЛ НЕ В(&МассивКодыМобилизованные)
	|				
	|ГДЕ
	|	ВзносыВФонды.Результат <> 0 ИЛИ ВзносыВФонды.РезультатСПределом <> 0 ИЛИ ВзносыВФонды.Налог <> 0
	|	ИЛИ ЕСТЬNULL(ВзносыФОТ.ДоначислениеФОТ, 0) <> 0 ИЛИ ЕСТЬNULL(ВзносыФОТ.НалогФОТ, 0) <> 0 ИЛИ ЕСТЬNULL(ВзносыФОТ.ДоначислениеНалогФОТ, 0) <> 0 
	|   ИЛИ ВЫБОР 
	|    КОГДА 
	|   	 ВзносыВФонды.ТипНачисления = 0 И ВзносыВФонды.КодКатегорииЗЛ НЕ В (&МассивКодыГПХ) И ВзносыВФонды.КодКатегорииЗЛ НЕ В (&МассивКодыБольничные) И ВзносыВФонды.Период = &ПериодОтчета 
	|	 ТОГДА ЕСТЬNULL(Отработанные.ОтработанныеДни,0)
	...
	|  ФизЛицоФио, КодПоДРФО, ВзносыВФонды.Период, КодКатегорииЗЛ, ТипНачисления
	|";
	...
	Если РегламентированнаяОтчетность.ИДКонфигурации() = "БП" Тогда
		...
		ТекстЗапросаВТФОТ_01052015 = СтрЗаменить(ТекстЗапросаВТФОТ_01052015,"РегистрРасчета.ВзносыВФонды","РегистрРасчета.ВзносыВФондыИНалоги");
	КонецЕсли;
	Если РегламентированнаяОтчетность.ИДКонфигурации() = "УТП" Тогда
	...
	Попытка
		Запрос.Текст    = ТекстЗапросаВТФОТ_01052015;
		Запрос.Выполнить();
	Исключение	
	...
	Пока ВыборкаТаблица6.Следующий() Цикл
		...
		ДобавленнаяСтрока[мИдГруппы6+"ДРФО"] 			= ОбработатьДРФО(ВыборкаТаблица6.КодПоДРФО);
		...
		Если Результат <> 0 И РезультатОтпускные <> 0 И ВыборкаТаблица6.Период = НачалоМесяца(мДатаКонцаПериодаОтчета) Тогда
			...
			ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовУдержано"]  = Налог;
			ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовНачислено"]  = НалогФОТ;
		...
		Иначе
			...
			ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовУдержано"]  = ВыборкаТаблица6.Налог;
			Если Результат = 0 И РезультатОтпускные <> 0 Тогда 
				//Это отпускные за следующий период
				ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовНачислено"]  = НалогФОТОтпускные;
			Иначе	
				ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовНачислено"]  = НалогФОТ;
			КонецЕсли;	
		КонецЕсли;	
		...
		ДобавленнаяСтрока[мИдГруппы6+"НеполноеРВ"] 	     = ВыборкаТаблица6.СокращенноеРабочееВремя;
		...
		Если Результат <> 0 И РезультатОтпускные <> 0 И ВыборкаТаблица6.Период = НачалоМесяца(мДатаКонцаПериодаОтчета) Тогда
			...
			ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовУдержано"]  = НалогОтпускные;
			ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовНачислено"]  = НалогФОТОтпускные;
		...
		Если ДоначислениеФОТ <> 0 ИЛИ ДоначислениеНалогФОТ <> 0 Тогда
			Если НомСтроки <= ИсходноеКоличествоСтрокГруппы6 Тогда
				ДобавленнаяСтрока = СтруктураГруппы6[НомСтроки - 1];
			Иначе
				Если ДобавленнаяСтрока[мИдГруппы6+"СуммаНачислено"] = 0 И ДобавленнаяСтрока[мИдГруппы6+"СуммаНачисленоМаксБаза"] = 0 И ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовУдержано"] = 0 Тогда
			...
			ДобавленнаяСтрока[мИдГруппы6+"Категория"] 	= ВыборкаТаблица6.КодКатегорииЗЛ;
	        ДобавленнаяСтрока[мИдГруппы6+"ТипСтавки"] 	= 13;
			...
			ДобавленнаяСтрока[мИдГруппы6+"СуммаНачислено"]= 0;
	        ДобавленнаяСтрока[мИдГруппы6+"СуммаНачисленоМаксБаза"] = 0;
			
			ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовУдержано"]  = 0;
			ДобавленнаяСтрока[мИдГруппы6+"СуммаВзносовНачислено"]  = ДоначислениеНалогФОТ;
			
			ДобавленнаяСтрока[мИдГруппы6+"СуммаРазницы"]  = ДоначислениеФОТ;
...
Процедура ЗаполнитьОтображениеДанныхВТаблицах()
	...
	ЛиквЕДРПОУ = мПереченьТаблиц.Области.ЛиквЕДРПОУ.Значение + "          ";
	...
	Коэффициент = мСлужебныеДанные.Области.Коэффициент.Значение;	           
	Если Коэффициент > 0 Тогда
		Коэффициент = "0" + Прав(Формат(Коэффициент, "ЧЦ=4; ЧДЦ=3"), 3);
	Иначе	
		Коэффициент = "";
	КонецЕсли;
	
	ЗначениеВКлетку(ЕДРПОУ, Ложь,   10, "ЕДРПОУ", мПереченьТаблиц);
	ЗначениеВКлетку(ЛиквЕДРПОУ, Ложь,   10, "ЛиквЕДРПОУ", мПереченьТаблиц);
	...
	ЗначениеВКлетку(ЛиквЕДРПОУ, Ложь,   10, "ЛиквЕДРПОУ", мТаблица1);
	
	мТаблица1.Области.НазваниеОрганизации.Значение = НазваниеОрганизации;
	
	ЗначениеВКлетку(Коэффициент,    Ложь, 4, "Коэффициент",    мТаблица1);
...
Процедура ПолеТабличногоДокументаПереченьТаблицВыбор(Элемент, Область, СтандартнаяОбработка)
	...
	Если ЛЕВ(Область.Имя, 6) = "ЕДРПОУ" Тогда
	...
	ИначеЕсли ЛЕВ(Область.Имя, 10) = "ЛиквЕДРПОУ" Тогда
		...
		Если ВвестиСтроку(ЛиквЕДРПОУ,  НСтр("ru = 'Укажите код ликвидинованной организации'; uk = 'Введіть код ліквідованої організації'"), 10, Ложь) Тогда
			мПереченьТаблиц.Области.ЛиквЕДРПОУ.Значение = ЛиквЕДРПОУ;
			ЗаполнитьОтображениеДанныхВТаблицах();
		КонецЕсли;
		
		СтандартнаяОбработка = Ложь;
		
	ИначеЕсли    Область.Имя = "Начальная" Тогда
	...
	ИначеЕсли Область.Имя = "Отменяющая" Тогда
		...
		мПереченьТаблиц.Области.Начальная.Значение = "";
		мПереченьТаблиц.Области.Отменяющая.Значение = "Х";
		мПереченьТаблиц.Области.Дополнительная.Значение = "";
	...
	ИначеЕсли Область.Имя = "Дополнительная" Тогда
		...
		мПереченьТаблиц.Области.Начальная.Значение = "";
		мПереченьТаблиц.Области.Отменяющая.Значение = "";
		мПереченьТаблиц.Области.Дополнительная.Значение = "Х";
...
Процедура ПолеТабличногоДокументаТаблица1ПриИзмененииСодержимогоОбласти(Элемент, Область)
	...
	Если НЕ ЗначениеЗаполнено(мТаблица1.Области.КоэффициентДа.Значение) Тогда
		мТаблица1.Области.СредняяЗП2014.Значение = 0;	
		мТаблица1.Области.СреднееЗЛ2014.Значение = 0;
	КонецЕсли;
...
Процедура ВыгрузитьЛистВDBF(СтруктураПараметровЛиста)
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица6" Тогда
		...
		Для каждого СтрокаГруппы Из СтруктураГруппы Цикл
			...
			докЭкспорт.SUM_DIFF	= СтрокаГруппы.Т6_СуммаРазницы;
			докЭкспорт.SUM_INS  = СтрокаГруппы.Т6_СуммаВзносовУдержано;
			докЭкспорт.SUM_NARAH= СтрокаГруппы.Т6_СуммаВзносовНачислено;
			докЭкспорт.OTK		= СтрокаГруппы.Т6_ТрудКнижка;
			докЭкспорт.EXP		= СтрокаГруппы.Т6_СпецСтаж;
			
			докЭкспорт.NRC		= СтрокаГруппы.Т6_НеполноеРВ;
...
Процедура ПолеТабличногоДокументаТаблица1Выбор(Элемент, Область, СтандартнаяОбработка)
	...
	ИначеЕсли Врег(Лев(Область.Имя, 11)) = Врег("Коэффициент") Тогда
		
		Если Врег(Прав(Область.Имя,2)) = "ДА" Тогда
			мТаблица1.Области.КоэффициентДа.Значение = "Х";
			мТаблица1.Области.КоэффициентНет.Значение = "";
			СтандартнаяОбработка = Ложь;
		ИначеЕсли Врег(Прав(Область.Имя,2)) = "ЕТ" Тогда
			
			мТаблица1.Области.КоэффициентДа.Значение = "";
			мТаблица1.Области.КоэффициентНет.Значение = "Х";
			мТаблица1.Области.СредняяЗП2014.Значение = 0;
			мТаблица1.Области.СреднееЗЛ2014.Значение = 0;
			
			мСлужебныеДанные.Области.Коэффициент.Значение = 0;
			ЗаполнитьОтображениеДанныхВТаблицах();
			
			СтандартнаяОбработка = Ложь;
			
		ИначеЕсли ЗначениеЗаполнено(мТаблица1.Области.КоэффициентДа.Значение) Тогда
			
			Коэффициент = мСлужебныеДанные.Области.Коэффициент.Значение;	
			
			Если ВвестиЧисло(Коэффициент,  НСтр("ru = 'Введите коэффициент (от 0,400 до 0,999)'; uk = 'Введіть коеффіцієнт (від 0,400 до 0,999)'"), 5, 3) Тогда
...
СтруктураГруппы6.Колонки.Добавить("Т6_ФИО", 		   ОписаниеТиповСтрока);
...
СтруктураГруппы6.Колонки.Добавить("Т6_СуммаРазницы",   		   ОписаниеТиповЧисло);
СтруктураГруппы6.Колонки.Добавить("Т6_СуммаВзносовУдержано",   ОписаниеТиповЧисло);
СтруктураГруппы6.Колонки.Добавить("Т6_СуммаВзносовНачислено",  ОписаниеТиповЧисло);
...
СтруктураГруппы6.Колонки.Добавить("Т6_НеполноеРВ",     ОписаниеТиповЧисло0);

СтруктураГруппы6.Колонки.Добавить("Т6_НовоеРМ", 	   ОписаниеТиповЧисло2);
...
СтруктураГруппы7.Колонки.Добавить("Т7_ФИО", 		 ОписаниеТиповСтрока);
