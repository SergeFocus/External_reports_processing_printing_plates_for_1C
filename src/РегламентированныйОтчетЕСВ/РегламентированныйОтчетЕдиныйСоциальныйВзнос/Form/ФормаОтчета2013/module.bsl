...
Процедура ЗаполнитьНачальныеНастройки() Экспорт
	...
	МакетСоставаПоказателей = ЭтотОбъект.ПолучитьМакет("СоставПоказателей2013");
...
Процедура ВывестиНаПечатьЛист(СтруктураПараметровЛиста)
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица5" Тогда
		...
		Макет = ПолучитьМакет("Таблица5_2013");
	...
	ИначеЕсли ИмяТабличногоПоля = "ПолеТабличногоДокументаТаблица6" Тогда
		...
		Макет = ПолучитьМакет("Таблица6_2013");
	...
	Для Каждого стр ИЗ Работавшие Цикл
		...
		Если ЗначениеЗаполнено(ДатаНач) Тогда
			...
			Исключение
				ДатаСозданияРМ = 0;
			КонецПопытки;	
			мТаблица5.Области[мИдГруппы5 + Формат(Номер,"ЧЦ=4;ЧВН=0;ЧГ=;") + "ДатаСозданияРМ"].Значение  = ДатаСозданияРМ;
		КонецЕсли;		
...
СтруктураГруппы6.Колонки.Добавить("Т6_КвоТрудОтношений", ОписаниеТиповЧисло0);
...
мСоставПоказателей = "СоставПоказателей2013";
