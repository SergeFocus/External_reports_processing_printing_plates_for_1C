	...
	Запрос.УстановитьПараметр("СчетДт", ПланыСчетов.Хозрасчетный.РасчетыПоПретензиям);
	Запрос.УстановитьПараметр("СчетКт", ПланыСчетов.Хозрасчетный.ПрочиеДоходыОтФинансовыхОпераций);
...
Пока РЗ.Следующий() Цикл
   ...
   Для Каждого ЗаписьБУ Из НаборЗаписейБУ Цикл
      Если (ЗаписьБУ.СчетДт = ПланыСчетов.Хозрасчетный.РасчетыПоПретензиям) и
          (ЗаписьБУ.СчетКт = ПланыСчетов.Хозрасчетный.ПрочиеДоходыОтФинансовыхОпераций) Тогда
        ЗаписьБУ.СчетКт = ПланыСчетов.Хозрасчетный.ПолученныеШтрафыПениНеустойки;
