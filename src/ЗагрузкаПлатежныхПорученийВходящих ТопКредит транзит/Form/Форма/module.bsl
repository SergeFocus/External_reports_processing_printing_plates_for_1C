...
Функция СоздатьППВходящее(ТабличныйДокумент, ТекущаяСтрока)	
	...
	Попытка	
		...
		Если  БухСчет = "3771"  Тогда
			...
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", РасчСчет);
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Погашення кредиту через debet Platon"));
			...
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Договоры", ДоговорКонтрагента);
		...
		ИначеЕсли БухСчет = "373" Тогда
			...
			РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3331");
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", РасчСчет);
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Погашення відсотків  через debet Platon"));
			РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("373");
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Контрагенты", Контрагент);
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Договоры", ДоговорКонтрагента);
			РегЗапись.Сумма = СуммаКредита;
			РегХозрасчетныйДеньгиВпути.Записать();
		...
		ИначеЕсли БухСчет = "374" Тогда
			...
			РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3331");
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", РасчСчет);
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("Погашення штрафів через debet Platon"));
			РегЗапись.СчетКт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("374");
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Контрагенты", Контрагент);
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "Договоры", ДоговорКонтрагента);
			РегЗапись.Сумма = СуммаКредита;
			РегХозрасчетныйДеньгиВпути.Записать();
		...
		Если  БухСчет = "373"  Тогда
			Если ОперацияНачислениеПроцентов = Неопределено Или ОперацияДеньгиВпути.Дата <>  НачалоДня(ДатаОперации)+4*60*60  Тогда
				ДокОперация = Документы.ОперацияБух.СоздатьДокумент();
				ДокОперация.Дата =  НачалоДня(ДатаОперации)+4*60*60;  
				ДокОперация.Организация = Организация;
				ДокОперация.Ответственный = ТекПользователь;
			...
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Контрагенты", Контрагент);
			ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "Договоры", ДоговорКонтрагента);
			//ОбщегоНазначения.УстановитьСубконто(РегЗапись.СчетКт, РегЗапись.СубконтоКт, "СтатьиДоходов", Справочники.СтатьиДоходов.НПНК_ДО_ );
			...
			Опер.Записать();
		ИначеЕсли БухСчет = "374" Тогда
			Если ОперацияНачислениеПені = Неопределено Или ОперацияДеньгиВпути.Дата <>  НачалоДня(ДатаОперации)+4*60*60  Тогда
				ДокОперация = Документы.ОперацияБух.СоздатьДокумент();
				ДокОперация.Дата =  НачалоДня(ДатаОперации)+4*60*60;  
				ДокОперация.Организация = Организация;
				ДокОперация.Ответственный = ТекПользователь;
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Организация = Справочники.Организации.НайтиПоКоду("000000006")
