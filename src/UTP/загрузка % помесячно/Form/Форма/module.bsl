...
Функция СоздатьППВходящее(ТабличныйДокумент, ТекущаяСтрока)	
	...
	//Датакредита = ПреобразоватьВДату(ТекущаяСтрока.Датакредита);
//	ФИО	= ТекущаяСтрока.ФИО;
	Начисленно =	формат(ТекущаяСтрока.Начисленно,"ЧЦ=7; ЧГ=2");
	
	НачатьТранзакцию();
	Попытка	
		ДоговорКонтрагента = Справочники.ДоговорыКонтрагентов.НайтиПоНаименованию(N_кредита);
		//Датакредита = ДоговорКонтрагента.Дата;
		...
		Если ОперацияНачислениеПроцентов = Неопределено  Тогда    //Или Месяц <> НачалоМесяца(Датакредита)
			//Месяц = НачалоМесяца(Датакредита) ;
			...
			ДокОперация.Содержание = "начисление процентов ПОМЕСЯЧНО " + Формат(Дата, "ДФ=""ММ""" ) ;
		...
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.РасчетыПоНачисленнымДоходам;
...
Процедура СформироватьСтруктуруКолонок() Экспорт
	...
	Колонка = Новый Структура;
	Колонка.Вставить("НомерКолонки",НомерКолонки);
	Колонка.Вставить("ИмяРеквизита","Начисленно");
...
Процедура ПередОткрытием(Отказ, СтандартнаяОбработка)
	...
	Дата = НачалоМесяца(ТекущаяДата())-2;
КонецПроцедуры
