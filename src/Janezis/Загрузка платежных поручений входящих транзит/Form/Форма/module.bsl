 ...
 РегХозрасчетныйНачислениеПроцентов = Неопределено;
	...
	Если Вопрос("Загрузить "+КоличествоЭлементов  + ТекстВопросаИсточника, РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		...
		Для НомерСтроки = ПерваяСтрокаДанныхТабличногоДокумента По ТабличныйДокумент.ВысотаТаблицы Цикл
			...
			если НЕ СоздатьДокументОперация(ТабличныйДокумент, ТекущаяСтрока)Тогда
...
Функция СоздатьДокументОперация(ТабличныйДокумент, ТекущаяСтрока)	
	...
	Если  БухСчет = "3771"  Тогда
		...
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", Справочники.БанковскиеСчета.НайтиПоРеквизиту("НомерСчета", "29247827503037"));
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("1111 Погашення кредиту через Crediton debet Platon"));
	...
	ИначеЕсли БухСчет = "373" Тогда
		...
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3331");
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", Справочники.БанковскиеСчета.НайтиПоРеквизиту("НомерСчета", "29247827503037"));
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("1112 Відсотки за кредитом debet Platon"));
	...
	ИначеЕсли БухСчет = "374" Тогда
		...
		РегЗапись.СчетДт = ПланыСчетов.Хозрасчетный.НайтиПоКоду("3331");
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "БанковскиеСчета", Справочники.БанковскиеСчета.НайтиПоРеквизиту("НомерСчета", "29247827503037"));
		БухгалтерскийУчет.УстановитьСубконто(РегЗапись.СчетДт, РегЗапись.СубконтоДт, "СтатьиДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоНаименованию("1113 Штраф за прострочення кредиту debet Platon"));
	...
	//
//////////////////////////////////////////////////////////////	
	СоздатьДокументОперацияЧасть2(N_кредита, БухСчет, ДатаОперации, ДоговорКонтрагента, Контрагент, СуммаКредита, ТекПользователь);
...
Процедура СоздатьДокументОперацияЧасть2(Знач N_кредита, Знач БухСчет, Знач ДатаОперации, ДоговорКонтрагента, Контрагент, Знач СуммаКредита, Знач ТекПользователь)
	
	Перем ДокОперация, Опер, РегЗапись;
	
	Если  БухСчет = "373"  Тогда
		Если ОперацияНачислениеПроцентов = Неопределено Или ОперацияНачислениеПроцентов.Дата <>  НачалоДня(ДатаОперации)+4*60*60 Тогда
			ДокОперация = Документы.ОперацияБух.СоздатьДокумент();
			ДокОперация.Дата =  НачалоДня(ДатаОперации)+4*60*60;  
			ДокОперация.Организация = глЗначениеПеременной("ОсновнаяОрганизация");
			ДокОперация.Ответственный = ТекПользователь;
		...
		РегЗапись.Период = ОперацияНачислениеПроцентов.Дата;
		...
		Опер.Записать();
	ИначеЕсли БухСчет = "374" Тогда
		Если ОперацияНачислениеПені = Неопределено Или ОперацияНачислениеПені.Дата <>  НачалоДня(ДатаОперации)+4*60*60 Тогда
			ДокОперация = Документы.ОперацияБух.СоздатьДокумент();
			ДокОперация.Дата =  НачалоДня(ДатаОперации)+4*60*60;  
			ДокОперация.Организация = глЗначениеПеременной("ОсновнаяОрганизация");
			ДокОперация.Ответственный = ТекПользователь;
		...
		РегЗапись.Период = ОперацияНачислениеПені.Дата;
		РегЗапись.Регистратор = ОперацияНачислениеПені;
		РегЗапись.Организация = глЗначениеПеременной("ОсновнаяОрганизация");
